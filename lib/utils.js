...
import { thisWeek } from "./constants.js";
...
export const getAppointmentsWeeklyStatus = (appointments) => {
...
export const newUsersThisWeek = (users) => {
  return users.reduce((count, user) => {
    const userJoinedAt = moment(user.joinedAt, "YYYY-MM-DD");

    if (userJoinedAt.isBetween(thisWeek.startDate, thisWeek.endDate, null, []))
      return (count += 1);
